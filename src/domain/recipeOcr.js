export function cleanRecipeOcrText(value, titleHint = "") {
...
function normalizeOcrIngredientRows(ingredientLines) {
  return ingredientLines
    ...
    .map((line) => {
      ...
      const match = normalized.match(/^((?:\d+(?:\s+\d\/\d|\/\d)?|\d+\s*\([^)]+\)|1\/4|1\/2|Salt)\s*(?:Tbsp|tsp|cups?|cloves?|bag|oz|lb|lbs|pounds?|to taste)?)(?:\s+)(.+)$/i);
      if (!match) {
...
function collectOcrInstructionSteps(lines) {
  const steps = [];
  let current = null;
  const hasInstructionHeading = lines.some((line) => /^(?:INSTRUCTIONS|DIRECTIONS|METHOD|PREPARATION)\b/i.test(line));
  let inInstructionSection = !hasInstructionHeading;
  lines.forEach((line) => {
    if (/^(?:INSTRUCTIONS|DIRECTIONS|METHOD|PREPARATION)\b/i.test(line)) {
      inInstructionSection = true;
      current = null;
      return;
    }
    if (!inInstructionSection || /^(?:NOTES|NUTRITION|PER SERVING)\b/i.test(line)) {
      return;
    }
